...
from student_management_app.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, \
    ...
    AssignmentReportStaff, QuizReportStaff
...
def staff_subject_details(request):
    staff=Staffs.objects.get(admin=request.user.id)
    notifications=NotificationStaffs.objects.filter(staff_id=staff.id)
    return render(request,"staff_template/staff_subject_details.html",{"notifications":notifications})

def staff_subjects(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    return render(request, 'staff_template/staff_subjects.html', {"subjects":subjects})

def staff_take_assignment(request):
    staff_obj = Staffs.objects.get(admin=request.user.id)
    assignment_data = AssignmentReportStaff.objects.filter(staff_id=staff_obj)

    subjects = Subjects.objects.filter(staff_id=request.user.id)
    session_years = SessionYearModel.object.all()
    return render(request, "staff_template/student_assignment_view.html",
                  {"subjects": subjects, "session_years": session_years,"assignment_data":assignment_data})
def staff_take_assignment_save(request):
    if request.method!="POST":
        return HttpResponseRedirect(reverse("staff_take_assignment"))
    else:
        subject_name = request.POST.get('subject')
        assignment_date=request.POST.get("assignment_date")
        assignment_msg=request.POST.get("assignment_msg")
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            assignment_report=AssignmentReportStaff(staff_id=staff_obj,subject_name=subject_obj,assignment_date=assignment_date,assignment_message=assignment_msg,assignment_file=assignment_file,assignment_status=0)
            assignment_report.save()
            messages.success(request, "Successfully Assignment Created")
            return HttpResponseRedirect(reverse("staff_take_assignment"))
        except:
...
def staff_take_quiz(request):
    staff_obj = Staffs.objects.get(admin=request.user.id)
    quiz_data = QuizReportStaff.objects.filter(staff_id=staff_obj)

    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years = SessionYearModel.object.all()
    return render(request, "staff_template/student_quiz_notification.html",
                  {"subjects": subjects, "session_years": session_years,"quiz_data":quiz_data})
def staff_take_quiz_save(request):
    if request.method!="POST":
        return HttpResponseRedirect(reverse("staff_take_quiz"))
    else:
        subject_name = request.POST.get('subject')
        quiz_date=request.POST.get("quiz_date")
        quiz_msg=request.POST.get("quiz_msg")

        # subject_obj = Subjects.objects.get(id=subject_name)

        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            quiz_report=QuizReportStaff(staff_id=staff_obj,subject_name=subject_name,quiz_date=quiz_date,quiz_message=quiz_msg,quiz_status=0)
            quiz_report.save()
            messages.success(request, "Successfully Quiz Created")
            return HttpResponseRedirect(reverse("staff_take_quiz"))
        except:
...
# def go_subject_details(request):
#     if request.method!='POST':
#      return HttpResponseRedirect('staff_subject_details')
...
def save_student_result(request):
    ...
    midterm_marks = request.POST.get('mid_marks')
    ...
    try:
        ...
        if check_exist:
            ...
            result.subject_midterm_marks=midterm_marks
            result.subject_final_marks = final_marks
