...
def student_subject(request):
    ...
    return render(request,"student_template/student_subject.html",{"studentresult":studentresult})

def s_quiz_view(request):
    student=Students.objects.get(admin=request.user.id)
    notifications=NotificationStudent.objects.filter(student_id=student.id)
    return render(request,"student_template/s_quiz_view.html",{"notifications":notifications})


def s_quizz(request):
    student=Students.objects.get(admin=request.user.id)
    notifications=NotificationStudent.objects.filter(student_id=student.id)
    return render(request,"student_template/s_quizz.html",{"notifications":notifications})

def student_assignment(request):
    student=Students.objects.get(admin=request.user.id)
    notifications=NotificationStudent.objects.filter(student_id=student.id)
    return render(request,"student_template/student_assignment.html",{"notifications":notifications})
...
def student_assignment_view(request):
    assignment=AssignmentReportStaff.objects.all()
    return render(request,"student_template/student_assignment_view.html",{"assignment":assignment})

# Student Quiz View

def student_quiz_view(request):
    quiz=QuizReportStaff.objects.all()
    return render(request,"student_template/student_quiz_view.html",{"quiz":quiz})
